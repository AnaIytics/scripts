...
local cfglocation = "astolfocfg/"
makefolder("astolfocfg")
...
function library:New(name)
	...
	astolfo.ZIndexBehavior = "Global"
	astolfo.DisplayOrder = 420133769
 
	local UIScale = Instance.new("UIScale")
	UIScale.Parent = astolfo
    ...
    game:GetService("RunService").RenderStepped:connect(function()
        cursor.Visible = astolfo.Enabled
	...
	game:GetService("UserInputService").InputBegan:Connect(function(key)
		if key.KeyCode == Enum.KeyCode.Insert then
			astolfo.Enabled = not astolfo.Enabled
			library.uiopen = astolfo.Enabled
		end
	end)
...
makefolder("astolfolua")
 
local allluas = {}
 
for _,lua in pairs(listfiles("astolfolua")) do
	local luaname = string.gsub(lua, "astolfolua\\", "")
...
luascripts:Element("Button", "load", {}, function()
	loadstring(readfile("astolfolua\\"..values.luas["lua scripts"].lua.Scroll))()
end)
...
chat:Element("Toggle", "chat spam", nil, function(tbl)
	if tbl.Toggle then
		while values.misc.chat["chat spam"].Toggle do
			game:GetService("ReplicatedStorage").Events.PlayerChatted:FireServer(values.misc.chat.type.Dropdown == "standard" and "hi" or "buy astolfo.lgbt", false, "Innocent", false, true)
			wait(values.misc.chat["speed (ms)"].Slider/1000)
		end
	end
end)
chat:Element("Dropdown", "type", {options = {"standard", "astolfo"}})
...
LocalPlayer.Status.Kills:GetPropertyChangedSignal("Value"):Connect(function(current)
	...
	if values.misc.chat["kill say"].Toggle then
		game:GetService("ReplicatedStorage").Events.PlayerChatted:FireServer(values.misc.chat["message"].Text ~= "" and values.misc.chat["message"].Text or "looks like you don't have astolfo.lgbt", false, "Innocent", false, true)
	end
end)
